...
export default function PromptsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/prompts?jobId=${jobUuid}&prompt=${encodeURIComponent(prompt)}&status=${status ? status : 'PENDING'}`)
  ...
  const startPolling = async (jobId: string) => {
    ...
    const pollInterval = setInterval(async () => {
      ...
      router.replace(`/prompts?jobId=${jobId}&prompt=${encodeURIComponent(prompt)}&status=${normalizedStatus}`,{ scroll: false })
    ...
    const timeoutId = setTimeout(() => {
      if (!isFinished) {
        ...
        router.replace(
        `/prompts?jobId=${jobId}&prompt=${encodeURIComponent(prompt)}&status=TIMEOUT`,
        { scroll: false }
      )
      }
    }, 10 * 60 * 1000)
  ...
  useEffect(() => {
    async function fetchVideoUrl() {
      ...
      if (videoUrl && urlExpiryTime && Date.now() < urlExpiryTime - 5 * 60 * 1000) {
